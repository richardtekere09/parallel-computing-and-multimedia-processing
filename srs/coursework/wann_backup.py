...
class WANNGenome:
    ...
    for i, val in enumerate(inputs):
        if i < self.input_size:
            node_values[i] = float(val)  # Ensure it's a float
    ...
    for node_id, (layer, activation) in sorted_nodes:
        ...
        for (src, dst), enabled in self.connections.items():
            if dst == node_id and enabled and src in node_values:
                ...
                src_value = float(node_values[src])
                total_input += src_value * shared_weight
    ...
    for i in range(self.output_size):
        ...
        output_value = node_values.get(output_id, 0.0)
        outputs.append(float(output_value))  # Ensure it's a float
