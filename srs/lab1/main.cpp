...
int main(int argc, char **argv)
{
    if (argc < 2)
    {
        std::cerr << "Usage: " << argv[0] << " <video_file> [bins] [threshold] [threads]" << std::endl;
        return 1;
    }
...
#pragma omp parallel for schedule(static)
        ...
        std::cout << "Run " << (r + 1) << ": " << runTime << " seconds, Scenes Detected: " << sceneCount << std::endl;
    ...
    std::cout << "✅ Average processing time: " << totalTime / runs << " seconds.\n";
    std::cout << "Output saved in: output/lab1/\n";
    return 0;
}
